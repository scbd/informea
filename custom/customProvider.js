$C('$data.storageProviders.Custom.CustomProvider', $data.StorageProviderBase, null,
{
    constructor: function (cfg, ctx) {
        ...
        this.providerConfiguration = $data.typeSystem.extend({
            ...
            //obsolate
            localStoreName: 'JayData_Custom_Provider',
            databaseName: 'JayData_Custom_Provider',
        ...
        if (this.providerConfiguration.databaseName === 'JayData_Custom_Provider')
    ...
    initializeStore: function (callBack) {
        ...
        if (this.providerConfiguration.persistentData && window.localStorage && this.providerConfiguration.dbCreation !== $data.storageProviders.DbCreationType.DropAllExistingTables) {
            var localStoreName = this.providerConfiguration.databaseName || "JayData_Custom_Provider";
            var that = this;
        ...
        this.dataSource = { 'custom_sequence': {} };
        for(var index = 0;index<this.context._storageModel.length;index++){
            ...
            if(computedKeys.length === 1){
                ...
                if (this.supportedAutoincrementKeys[resolvedType] === true){
                    ...
                    this.dataSource['custom_sequence'][storageModel.TableName] = 0;
                    isIntegerPk = true;
            ...
            if (tempSource[storageModel.TableName]) {
                for (var i = 0; i < tempSource[storageModel.TableName].length; i++) {
                    ...
                    if(isIntegerPk){
                        ...
                        if (keyValue > this.dataSource['custom_sequence'][storageModel.TableName]) {
                            this.dataSource['custom_sequence'][storageModel.TableName] = keyValue;
                        }
                    }
    ...
    _save_add_processPk:function(item){
        ...
        if(keys.length === 1 && keys[0].computed){
            ...
            if (typeof this.supportedAutoincrementKeys[keyResolveType] === 'function') {
                ...
                var sequenceValue = this.dataSource['custom_sequence'][item.entitySet.tableName];
                item.data[key.name] = sequenceValue+1;
                this.dataSource['custom_sequence'][item.entitySet.tableName] = sequenceValue + 1;
            //}else{
    ...
    fieldConverter: { value: $data.CustomConverter },
...
$C('$data.storageProviders.Custom.LocalStorageProvider', $data.storageProviders.Custom.CustomProvider, null,{
    constructor:function(cfg, ctx){
        this.providerConfiguration.persistentData = true;
    }
}, null);
$data.StorageProviderBase.registerProvider("Custom", $data.storageProviders.Custom.CustomProvider);
$data.StorageProviderBase.registerProvider("LocalStore", $data.storageProviders.Custom.LocalStorageProvider);
